...
function loadAllocations($conn, $uid) {
    ...
    foreach ($rows as &$r) {
        $r['id']        = (int)$r['id'];
        $r['remaining'] = round((float)$r['allocated_amount'] - (float)$r['amount_used'], 2);
    }
    unset($r);
    return $rows;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$alloc) {
        ...
        $live_remaining = round((float)$alloc['allocated_amount'] - (float)$alloc['amount_used'], 2);
    ...
    if ($msgType !== 'danger') {
        ...
        // Reserve the amount immediately (reversed on admin rejection)
        $conn->query("UPDATE budget_allocations SET amount_used = amount_used + $total WHERE id = $alloc_id");

        logActivity($conn, 'SUBMIT_PURCHASE',
            "Purchase #$newPid: $itemName x$qty @ ₱$price = ₱$total | alloc #$alloc_id | budget #{$alloc['budget_id']}");

        $sharedNote = $alloc['is_shared'] ? ' (shared pool)' : '';
        $msg = 'Purchase submitted! ₱' . number_format($total, 2)
             . ' reserved from "' . htmlspecialchars($alloc['allocation_title'] ?? 'Allocation')
             . '"' . $sharedNote . '. Awaiting admin review.';
        $msgType = 'success';
...
<div class="grid-2" style="gap:24px;">
    <div class="card">
        ...
        <div style="margin-bottom:20px;padding:14px;background:var(--cream);border-radius:10px;border:1px solid var(--cream-dark);">
            ...
            <select id="allocSelect" class="form-control" onchange="updateBudgetInfo(this)">
                ...
                <?php
                $hasPersonal = false; $hasShared = false;
                foreach ($myAllocations as $a) {
                    if ($a['is_shared']) $hasShared = true; else $hasPersonal = true;
                }
                if ($hasPersonal): ?>
                <optgroup label="── Personal Allocations">
                <?php foreach ($myAllocations as $a): if (!$a['is_shared'] && $a['remaining'] > 0): ?>
                    <option value="<?= $a['id'] ?>"
                        ...
                        data-start="<?= $a['budget_start'] ?>" data-end="<?= $a['budget_end'] ?>"
                        data-shared="0">
                ...
                <?php endif; ?>
                <?php if ($hasShared): ?>
                <optgroup label="── Shared Pool Allocations">
                <?php foreach ($myAllocations as $a): if ($a['is_shared'] && $a['remaining'] > 0): ?>
                    <option value="<?= $a['id'] ?>"
                        data-remaining="<?= $a['remaining'] ?>"
                        data-title="<?= htmlspecialchars(addslashes($a['allocation_title'] ?? 'Shared')) ?>"
                        data-period="<?= htmlspecialchars($a['period_label']) ?>"
                        data-start="<?= $a['budget_start'] ?>" data-end="<?= $a['budget_end'] ?>"
                        data-shared="1">
                        [Shared] <?= htmlspecialchars($a['allocation_title'] ?? 'Shared Allocation') ?>
                        — <?= formatCurrency($a['remaining']) ?> remaining
                    </option>
                <?php endif; endforeach; ?>
                </optgroup>
            ...
            <div id="budgetInfoBox" style="display:none;margin-top:14px;">
                <div style="display:flex;gap:16px;flex-wrap:wrap;font-size:13px;margin-bottom:8px;">
                    <div><span style="color:var(--text-muted)">Period:</span> <strong id="biPeriod">—</strong></div>
                    <div><span style="color:var(--text-muted)">Available:</span> <strong id="biRemaining" style="color:var(--success)">—</strong></div>
                    <div><span style="color:var(--text-muted)">This Purchase:</span> <strong id="biTotal" style="color:var(--primary)">₱0.00</strong></div>
                    <div><span style="color:var(--text-muted)">After:</span> <strong id="biAfter">—</strong></div>
                </div>
                <div class="progress-bar" style="height:8px;">
                    <div class="progress-fill green" id="biBar" style="width:0%;transition:width .3s,background .3s;"></div>
                </div>
                <div id="biSharedNote" style="display:none;margin-top:8px;padding:7px 12px;background:rgba(59,130,246,.08);border-radius:8px;font-size:12px;color:#1d4ed8;">
                    <i class="fas fa-users"></i> This is a <strong>shared pool</strong> — balance shown is the remaining total for all encoders.
                </div>
                <div id="biWarning" style="display:none;margin-top:8px;padding:8px 12px;background:rgba(185,28,28,.08);border-radius:8px;color:var(--danger);font-size:13px;font-weight:600;">
                    <i class="fas fa-exclamation-triangle"></i> Purchase total exceeds available balance!
                </div>
            </div>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" id="purchaseForm">
            ...
            <div class="form-group">
                ...
                <input type="text" name="item_name" id="itemNameInput" class="form-control" required
                       placeholder="e.g. All-Purpose Flour" oninput="checkInventory(this.value)">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="quantity" id="qtyInput" class="form-control"
                           required step="0.01" min="0.01" placeholder="0.00" oninput="recalcTotal()">
                </div>
                <div class="form-group">
                    <label>Unit *</label>
                    <input type="text" name="unit" id="unitInput" class="form-control" required placeholder="kg, pcs, L…">
                </div>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" name="unit_price" id="priceInput" class="form-control"
                           required step="0.01" min="0.01" placeholder="0.00" oninput="recalcTotal()">
                </div>
                <div class="form-group">
                    <label>Total Amount</label>
                    <div id="totalDisplay" style="
                        background:var(--cream);border:2px solid var(--primary);border-radius:10px;
                        padding:10px 16px;font-size:22px;font-weight:800;font-family:'Fraunces',serif;
                        color:var(--forest);text-align:right;letter-spacing:-0.02em;
                        transition:border-color .2s,color .2s;">₱0.00</div>
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="supplier" class="form-control" placeholder="Supplier name or store">
                </div>
            </div>
    ...
    <div style="display:flex;flex-direction:column;gap:20px;">
        <div class="card">
            ...
            ?>
            <div style="margin-bottom:12px;padding:12px;border:1px solid var(--grey-200);border-radius:10px;<?= $a['remaining']<=0?'opacity:.5;':'' ?>">
                <div style="display:flex;justify-content:space-between;margin-bottom:4px;">
                    <span style="font-weight:600;font-size:13px;"><?= htmlspecialchars($a['allocation_title']??'Allocation') ?></span>
                    <?php if ($a['is_shared']): ?>
                    <span class="badge badge-pending" style="font-size:10px;">Shared Pool</span>
                    <?php endif; ?>
                </div>
                <div style="font-size:11px;color:var(--text-muted);margin-bottom:6px;">
                    <?= htmlspecialchars($a['period_label']) ?> · <?= $a['budget_start'] ?> to <?= $a['budget_end'] ?>
                </div>
                <div class="progress-bar" style="height:6px;margin-bottom:6px;">
                    <div class="progress-fill <?= $pct>=90?'red':($pct>=70?'orange':'green') ?>" style="width:<?= $pct ?>%"></div>
                </div>
                <div style="display:flex;gap:12px;font-size:12px;color:var(--text-muted);">
                    <span>Alloc: <strong style="color:var(--text)"><?= formatCurrency($a['allocated_amount']) ?></strong></span>
                    <span>Used: <strong style="color:var(--danger)"><?= formatCurrency($a['amount_used']) ?></strong></span>
                    <span>Left: <strong style="color:<?= $a['remaining']>0?'var(--success)':'var(--danger)' ?>"><?= formatCurrency($a['remaining']) ?></strong></span>
                </div>
            </div>
            <?php endforeach; ?>
            <?php endif; ?>
        ...
        <div class="card">
            ...
            <p style="font-size:13px;color:var(--text-muted);margin-bottom:14px;">Check existing stock before purchasing.</p>
...
function updateBudgetInfo(sel) {
    ...
    document.getElementById('allocIdInput').value = '';
    currentRemaining = 0;
    ...
    document.getElementById('allocIdInput').value = opt.value;
    ...
    document.getElementById('biSharedNote').style.display = opt.dataset.shared === '1' ? 'block' : 'none';
...
function recalcTotal() {
    ...
    if (!allocChosen)        setSubmitState(false, 'Select an allocation above to enable this button.');
    else if (overBudget)     setSubmitState(false, 'Total exceeds available allocation balance.');
    else if (total <= 0)     setSubmitState(false, 'Enter a valid quantity and unit price.');
...
function checkInventory(val) {
    ...
    if (match && lower.length >= 3) {
        document.getElementById('stockHintText').textContent =
            'Existing stock: ' + match.stock + ' ' + match.unit + ' (minimum: ' + match.min + ').';
        const ui = document.getElementById('unitInput');
        if (!ui.value) ui.value = match.unit;
...
document.getElementById('purchaseForm')?.addEventListener('submit', function(e) {
    ...
    if (!allocId) { e.preventDefault(); alert('Please select a budget allocation.'); return; }
    const qty   = parseFloat(document.getElementById('qtyInput').value)   || 0;
    const price = parseFloat(document.getElementById('priceInput').value) || 0;
    const total = Math.round(qty * price * 100) / 100;
    if (currentRemaining > 0 && total > currentRemaining) {
        e.preventDefault();
        alert('Purchase total (' + fmtCur(total) + ') exceeds available balance (' + fmtCur(currentRemaining) + ').');
    }
});
</script>
